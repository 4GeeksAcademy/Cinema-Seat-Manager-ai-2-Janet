...
if (typeof document !== "undefined") {
  ...
  if (app) {
      ...
      <section class="mx-auto w-full max-w-6xl p-6">
        ...
        <section class="mb-5 rounded-2xl bg-white p-5 shadow-sm">
          <div class="flex flex-wrap gap-3">
            <button id="find-pair" class="rounded-lg bg-cyan-600 px-4 py-2 font-semibold text-white transition hover:bg-cyan-700">Find and Reserve 2 Adjacent Seats</button>
          ...
          <div class="mt-4 grid gap-2 sm:grid-cols-2">
            <p id="mode-text" class="rounded-lg bg-slate-100 p-3 text-sm font-medium">Single seat purchase: click any available seat.</p>
    ...
    if (!gridEl || !messageEl || !modeEl || !countEl || !findPairBtn || !resetBtn) {
    ...
    findPairBtn.addEventListener("click", () => {
      ...
      if (pair[0] === -1) {
        ...
        const reserveResult = reserveCoupleSeats(seating, pair[0], pair[1]);
        messageEl.textContent = reserveResult[1];
      }
    ...
    messageEl.textContent =
      "Click a seat to reserve one, or use Find and Reserve 2 Adjacent Seats for a couple purchase.";
    refreshUI();
  }
}
